...
var storage = function(spec, my) {
  ...
  put = function(path, json, cb_) {
    ...
    async.series([
      function(cb_) {
        lock(path + '._write', function(rel_) {
          release = rel_;
          return cb_();
        });
      },
      ...
      function(cb_) {
        ...
        var _write = require('path').join(my.data_path, path + '._write');
        fs.writeFile(_write, data, {
          encoding: 'utf8',
          flag: 'w'
        }, cb_);
      },
      function(cb_) {
        var _write = require('path').join(my.data_path, path + '._write');
        var _dest = require('path').join(my.data_path, path);
        fs.rename(_write, _dest, cb_);
      }
    ], function(err) {
      release();
      return cb_(err, json);
